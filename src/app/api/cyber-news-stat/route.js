...
export async function GET() {
  ...
  for (const field of fields) {
    ...
    result[field] = raw.map(({ _id, count }) => {
      const label = field === "countries" ? normalizeCountry(_id) : formatLabel(_id);
  ...
  result.mostUsedAttackType = result.attack_techniques?.[0]?._id || "N/A";
  result.mostTargetedSector = result.sectors?.[0]?._id || "N/A";
